...
class WOTS(AbstractOTS):
    ...
    def getPubkeyFromSignature(self, message: bytes,
        ...
        if len(signature) != self.__key_count:
            # return False
            raise BadSignature("Value {0} is not equal to {1}".format(len(signature), self.__key_count))
    ...
    def getPubkeyFromSignatureHash(self, msghash: bytes,
                               signature: List[bytes]) -> List[bytes]:
        if len(signature) != self.__key_count:
            # return False
            raise BadSignature("Value {0} is not equal to {1}".format(len(signature), self.__key_count))
